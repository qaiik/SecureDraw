...
class AnonymousDrawSystemCanvas {
    ...
    constructor(w, h, brushSize = 1, defaultColor = "black", minSize = 1) {
    ...
    drawCircle(size = 5, canvasX = 0, canvasY = 0) {
        ...
        // Log only if far enough from the last logged point
        if (!this.#lastLogged || this.#distance(this.#lastLogged, {
                x: canvasX,
                y: canvasY
            }) >= size / 2) {
            this.#log += `CIRCLE ${size} ${~~canvasX} ${~~canvasY}\n`; // Log without color
            this.#binaryLog.push(1, size, ...toUint16([~~canvasX, ~~canvasY]))
            this.#lastLogged = {
                x: canvasX,
                y: canvasY
            };
    ...
    loadFromBinary(binarylog, draw = true) {
        ...
        while (n < binarylog.length) {
            switch (binarylog[n]) {
                case 1:
                    const size = binarylog[++n]
                    const position = []
                    for (let i = 0; i < 4; i++) {
                        n++
                        position.push(binarylog[n])
                    }
                    // console.log(size, a,b,c,d)
                    this.drawCircle(size, ...fromUint16(position))
                    n++
                    break
                case 2:
...
class PublicDrawSystemCanvas {
    ...
    constructor(publicCanvas, brushSize = 1, defaultColor = "black", minSize = 1) {
